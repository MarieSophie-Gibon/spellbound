      ...
      <div className="space-y-3 flex-1">
          ...
          <div className="flex-1 min-h-66 max-h-66 overflow-y-auto border border-dashed border-[#E3CCCD]/25 rounded-2xl p-2 text-[11px] text-white/90 space-y-1.5">
            <div className="rounded-lg border border-white/10 bg-white/5 px-2 py-1.5">
              <div className="flex flex-col gap-1 text-[10px] text-white/90">
                <span className="uppercase tracking-widest text-[#E3CCCD]/65 line-clamp-1">
            ...
            <div className="rounded-lg border border-dashed border-[#E3CCCD]/25 px-2 py-2">
              <p className="text-[8px] uppercase tracking-widest text-[#E3CCCD]/65 mb-1.5">
                Caractéristiques
              </p>
              <div className="grid grid-cols-4 gap-1">
                {STAT_KEYS.map((k) => (
                  <div
                    key={k}
                    className="bg-white/5 border border-white/10 rounded-lg p-1.5 flex flex-col items-center gap-0.5"
                  >
                    <span className="text-[8px] uppercase tracking-wider text-white/60 font-semibold">
                      {STAT_LABELS[k]}
                    </span>
                    <span
                      className={`text-[11px] font-semibold leading-none ${monstre.stats[k].mod > 0 ? "text-emerald-400" : monstre.stats[k].mod < 0 ? "text-red-400" : "text-white"}`}
                    >
                      {monstre.stats[k].mod > 0
                        ? `+${monstre.stats[k].mod}`
                        : monstre.stats[k].mod}
                    </span>
                    {monstre.stats[k].sup && (
                      <span className="text-[7px] text-amber-400/90 uppercase tracking-wider border border-amber-400/25 rounded px-1">
                        bonus
                      </span>
                    )}
                  </div>
        ...
        {monstre.attaques.length > 0 && (
          <div className="space-y-2">
            <p className="text-[9px] uppercase tracking-widest text-[#E3CCCD]/65 px-1">
              Attaques
            </p>
    ...
    <div className="bg-[#29206A]/20 border border-[#E3CCCD]/20 rounded-2xl px-4 py-3 flex items-center gap-4">
      ...
      {attaque.dm && (
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className="text-[13px] font-mono text-[#E3CCCD]/80 font-semibold">
            {attaque.dm}
          </span>
        </div>
      )}
...
function CapaciteBlock({
  ...
  capacite: MonstreCapacite;
  defaultOpen?: boolean;
}) {
  const [open, setOpen] = useState(defaultOpen ?? false);

  const typeColor =
    capacite.type === "passif"
    ...
    <div className="bg-[#29206A]/20 border border-[#E3CCCD]/20 rounded-2xl overflow-hidden">
      ...
      >
        <span className="font-serif text-base text-white truncate">
          {capacite.nom || "Capacité sans nom"}
        </span>
        <div className="flex items-center gap-2 shrink-0">
          <span
            className={`text-[10px] font-bold tracking-wider px-2 py-0.5 rounded-md border ${typeColor}`}
          >
            {typeLabel}
          </span>
          <ChevronDown
