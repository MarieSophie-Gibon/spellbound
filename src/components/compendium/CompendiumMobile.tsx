...
export function CompendiumMobile({
  ...
  const [openBestiaireGroups, setOpenBestiaireGroups] = useState<Set<string>>(new Set());
  const wasListInView = useRef(showListInView);
  const normalizedQuery = query.trim().toLowerCase();
  const isBestiaireOnlyView = sections.length === 1 && sections[0] === "bestiaire";
  ...
  }, [showListInView, onSectionChange, isBestiaireOnlyView]);
  ...
  const toggleBestiaireGroup = (groupName: string) => {
    setOpenBestiaireGroups((prev) => {
      const next = new Set(prev);
      if (next.has(groupName)) next.delete(groupName);
      else next.add(groupName);
      return next;
  ...
  const bestiaireByType = useMemo(() => {
    const byQuery = (value: string) => !normalizedQuery || value.toLowerCase().includes(normalizedQuery);
    const filteredMonstres = monstres.filter((m) => byQuery(m.nom) || byQuery(m.type_creature ?? ""));

    const grouped = filteredMonstres.reduce<Record<string, Monstre[]>>((acc, monstre) => {
      const groupName = monstre.type_creature?.trim() || "Sans type";
      if (!acc[groupName]) acc[groupName] = [];
      acc[groupName].push(monstre);
      return acc;
    }, {});
    ...
    return Object.keys(grouped)
      ...
      .map((groupName) => ({
        groupName,
        monstres: grouped[groupName].sort((a, b) => a.nom.localeCompare(b.nom)),
      }));
  }, [monstres, normalizedQuery]);
          ...
          {showListInView ? (
            <div className="absolute inset-0 flex min-h-0 flex-col">
              ...
              <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-4 pt-3 space-y-2 scrollbar-thin scrollbar-thumb-white/20">
                {isBestiaireOnlyView ? (
                  <div className="rounded-xl border border-[#E3CCCD]/16 bg-white/6 overflow-hidden px-2 py-2 space-y-1">
                    {bestiaireByType.length === 0 ? (
                      <div className="px-3 py-2 text-[11px] text-white/40 italic">Aucun element dans cette categorie.</div>
                    ) : (
                      bestiaireByType.map(({ groupName, monstres }) => {
                        const isGroupOpen = openBestiaireGroups.has(groupName);

                        return (
                          <div key={groupName} className="overflow-hidden">
                            <button
                              type="button"
                              onClick={() => toggleBestiaireGroup(groupName)}
                            ...
                            <SmoothPanel open={isGroupOpen}>
                              <div className="mt-0.5 mb-1 ml-2 border-l border-white/10 pl-2 space-y-0.5">
                                {monstres.map((monstre) => (
                                  ...
                                  >
                                    <span className="block leading-snug line-clamp-2">{monstre.nom}</span>
                                  </button>
                                ))}
                              </div>
                            </SmoothPanel>
              ...
              {!readOnly && (openSection || isBestiaireOnlyView) && (
