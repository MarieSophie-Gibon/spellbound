...
import { Search, Dices, CheckCircle2, XCircle, Plus, Trash2, ChevronDown, ArrowUp, ArrowDown } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectGroup,
...
export function InvestigationBlock({ data, onChange, isEditing = true }: InvestigationBlockProps) {
  ...
  const moveTest = (fromIndex: number, toIndex: number) => {
    if (toIndex < 0 || toIndex >= tests.length || fromIndex === toIndex) return;
    const updated = [...tests];
  ...
  return (
    <div className="flex flex-col border border-sky-500/20 bg-sky-500/5 rounded-xl overflow-hidden shadow-lg">

      {/* HEADER COLLAPSIBLE */}
      <div className="flex items-center gap-2 px-3.5 md:px-4 py-2.5">
        <button
          onClick={() => setCollapsed((c) => !c)}
          className="w-7 h-7 rounded-lg bg-sky-500/10 border border-sky-500/20 flex items-center justify-center shrink-0 hover:bg-sky-500/20 transition-colors group"
        >
        ...
        {isEditing ? (
          <input
            ...
            className="flex-1 bg-transparent font-serif text-[15px] text-sky-100 outline-none placeholder:text-sky-100/30 min-w-0"
          />
        ) : (
          <span className="flex-1 font-serif text-[15px] text-sky-100 whitespace-pre-wrap wrap-break-word">
      ...
      {/* CONTENU */}
      {!collapsed && (
      <div className="p-3.5 md:p-4 pt-0 flex flex-col gap-3">
        ...
        {(isEditing || data.description) && (
          <textarea
            ...
            className="w-full bg-transparent text-white/70 text-[12px] leading-relaxed outline-none resize-none overflow-hidden min-h-9 placeholder:text-white/20 border-b border-sky-500/20 focus:border-sky-400/50 transition-colors"
            onInput={(e) => {
        ...
        <div className="flex flex-col gap-3.5">
          {tests.map((test, index) => {
            ...
            return (
            <div key={index} className="flex flex-col gap-2.5 border border-sky-500/15 rounded-lg p-2.5 bg-black/10">
              ...
              <div className="flex items-center gap-2">
                ...
                {tests.length > 1 && (
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      onClick={() => moveTest(index, index - 1)}
                      disabled={index === 0}
                      className="w-7 h-7 flex items-center justify-center rounded-lg text-white/30 hover:text-sky-300 hover:bg-sky-500/10 transition-colors disabled:opacity-20 disabled:cursor-not-allowed"
                      title="Monter le test"
                    >
              ...
              {/* MÉCANIQUE (JET ET DD) */}
              <div className="flex flex-wrap items-center gap-2.5 bg-black/20 border border-sky-500/10 rounded-lg px-2.5 py-2">
                <div className="flex items-center gap-2.5 flex-1 min-w-50">
                  <Select
                    value={currentStat}
                    onValueChange={(value) => updateTest(index, { stat: value })}
                  >
                    <SelectTrigger className="h-7.5 min-w-40 bg-black/30 border border-sky-500/20 rounded-lg px-2 py-1 text-[12px] text-white focus:border-sky-400/45 focus:ring-2 focus:ring-sky-400/15">
                      <SelectValue placeholder="PER">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="px-1.5 py-0.5 rounded-md bg-sky-500/10 border border-sky-500/20 text-[9px] uppercase tracking-wide font-semibold text-sky-200">
                            {currentStat}
                          </span>
                          <span className="text-[11px] text-white/70 whitespace-pre-wrap wrap-break-word">
                  ...
                  <div className="flex items-center gap-2 bg-black/30 border border-sky-500/20 rounded-lg px-2.5 py-1.5 focus-within:border-sky-400/50 transition-colors">
                    ...
                    <input
                      ...
                      className="w-7 bg-transparent text-[12px] text-white text-center outline-none font-mono"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                {/* SUCCÈS */}
                <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-lg p-2.5 flex flex-col gap-1.5">
                  <div className="flex items-center gap-2 text-emerald-400">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span className="text-[10px] uppercase tracking-widest font-bold">En cas de réussite</span>
                  </div>
                  <textarea
                    value={test.success || ""}
                    ...
                    className="w-full bg-transparent text-emerald-50/80 text-[12px] leading-relaxed outline-none resize-none overflow-hidden min-h-10 placeholder:text-emerald-500/30"
                    onInput={(e) => {
                      const target = e.target as HTMLTextAreaElement;
                      target.style.height = "auto";
                      target.style.height = `${target.scrollHeight}px`;
                ...
                {/* ÉCHEC */}
                <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-2.5 flex flex-col gap-1.5">
                  <div className="flex items-center gap-2 text-red-400">
                    <XCircle className="w-3.5 h-3.5" />
                    <span className="text-[10px] uppercase tracking-widest font-bold">En cas d'échec</span>
                  </div>
                  <textarea
                    value={test.failure || ""}
                    ...
                    className="w-full bg-transparent text-red-50/80 text-[12px] leading-relaxed outline-none resize-none overflow-hidden min-h-10 placeholder:text-red-500/30"
                    onInput={(e) => {
                      const target = e.target as HTMLTextAreaElement;
                      target.style.height = "auto";
                      target.style.height = `${target.scrollHeight}px`;
        ...
        <button
          onClick={addTest}
          className="flex items-center justify-center gap-2 w-full py-1.5 rounded-lg border border-dashed border-sky-500/20 text-sky-400/60 hover:text-sky-400 hover:border-sky-400/40 hover:bg-sky-500/5 transition-colors text-[11px] uppercase tracking-widest font-medium"
        >
