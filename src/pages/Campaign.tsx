...
export function CampaignHome({
  ...
  const [selectedPnjVoies, setSelectedPnjVoies] = useState<
    Array<{ id: string; nom: string; rang: number }>
  >([]);
  ...
  const selectedPnj =
    (revealedPnjs ?? []).find((pnj) => pnj.id === selectedPnjId) ?? null;
  ...
  useEffect(() => {
    const loadVoies = async () => {
      ...
      const voieIds = selectedPnj.pathways
        .map((p: { voie_id: string }) => p.voie_id)
        .filter(Boolean);
      if (voieIds.length === 0) {
        setSelectedPnjVoies([]);
        return;
      ...
      const voieName = new Map(
        (data ?? []).map((v: { id: string; nom: string }) => [v.id, v.nom]),
      );
      setSelectedPnjVoies(
        selectedPnj.pathways
          .map((p: { voie_id: string; rangs_acquis?: number[] }) => ({
            ...
            rang:
              p.rangs_acquis && p.rangs_acquis.length > 0
                ? Math.max(...p.rangs_acquis)
                : 0,
          }))
          .sort((a: { nom: string }, b: { nom: string }) =>
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 p-8 pr-4 flex flex-col gap-8">
        ...
        {membres.length > 0 && (
          <section className="rounded-xl border border-white/10 bg-[#1E1941]/55 backdrop-blur-lg p-4 w-44 h-80 absolute top-4 right-50">
      ...
      <div className="w-60 shrink-0 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 p-4 flex flex-col gap-3">
                ...
                <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      background:
                        "linear-gradient(90deg, #E3CCCD 0%, #c9a8aa 100%)",
                    }}
                  />
                </div>
                <span className="text-[10px] text-white/30">
                  {progress!.totalScenarios} scénario
                  {progress!.totalScenarios > 1 ? "s" : ""}
                </span>
              </div>
            </>
          )}
        ...
        {isMJ && (
          <div className="rounded-xl border border-amber-300/20 bg-amber-500/5 backdrop-blur-sm px-4 py-3 flex flex-col gap-2">
            ...
            {inviteError && (
              <p className="text-[10px] text-red-300">{inviteError}</p>
            )}
          </div>
        )}
          ...
          >
            <div className="px-5 py-4 border-b border-white/10 flex items-start gap-3">
              <div className="w-12 h-12 rounded-full border-2 border-violet-400/60 overflow-hidden shrink-0">
                <img
                  src={selectedPnj.image_url || "/default-avatar.png"}
                  alt={selectedPnj.name}
                  className="w-full h-full object-cover"
                />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[10px] uppercase tracking-widest text-violet-300/55">
                  Fiche technique PNJ
                </p>
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                {[
                  ...
                  {
                    label: "Att. contact",
                    val: selectedPnj.stats?.att_contact,
                    plus: true,
                  },
                  {
                    label: "Att. distance",
                    val: selectedPnj.stats?.att_distance,
                    plus: true,
                  },
                  {
                    label: "Att. magie",
                    val: selectedPnj.stats?.att_magie,
                    plus: true,
                  },
                ].map(({ label, val, plus }) => (
                  <div
